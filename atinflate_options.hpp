...
int S,A,Acap,T,Tcap,n,k,N,L,trial,fxhalt,nbase=0,seed,endfix=0;
int n_d_oall;
int n_d_wout;
int n_d_win;
...
double kdc, kdnc, epsilon, mu, Mu,phi,p,fthalt,kappa;
random_device rd;
map<double,int> aa_to_st;
mt19937_64 mt;
uniform_real_distribution<double> dist(0,1);
bool rate=true,b0=false,b1=false,codon_ring_space=true,bif=false;
...
int initialize_variables(int argc, char* argv[])
{
  ...
  for(int i=1;i<argc;i++){
    ...
    if(str_arg.substr(0,3) == "-k=")
      {
	k = stoi(str_arg.substr(3));
	if(k <= 0){
	  cout<<"\nk must be a positive integer. Use -h or --help for more.\n\n";
	  return 0;
	}
	bns=true;
      }
	...
	cout<<"Site string is "<<sitestring<<endl;
	bst=true;
      }
    ...
    if(str_arg.substr(0,7) == "--kdnc=")
      {
	kdnc=float(convertToDouble(str_arg.substr(7,str_arg.length()-1)));
	if(kdnc <= 0){
	  cout<<"\nkdnc must be in (0,infinity). Use -h or --help for more.\n\n";
	  return 0;
	}
	bkmax=true;
      }

        if(str_arg.substr(0,6) == "--kdc=")
      {
	kdc=float(convertToDouble(str_arg.substr(6,str_arg.length()-1)));
	if(kdc <= 0){
	  cout<<"\nkdc must be in (0,infinity). Use -h or --help for more.\n\n";
	  return 0;
	}
	bkmin=true;
      }
	...
	if(str_arg.substr(0,7) != "--seed=" && str_arg.substr(0,6) != "--phi=" && str_arg.substr(0,6) != "--kdc=" && str_arg.substr(0,7) != "--kdnc=" && str_arg.substr(0,5) != "--mu=" && str_arg.substr(0,9) != "--no-rate" && str_arg.substr(0,21) != "--uniform-amino-acids" && str_arg.substr(0,2) != "-m" && str_arg.substr(0,3) != "-o=" && str_arg.substr(0,8) != "--ofile=" && str_arg.substr(0,3) != "-T=" && str_arg.substr(0,8) != "--tRNAs=" && str_arg.substr(0,3) != "-A=" &&  str_arg.substr(0,13) != "--starting-A=" && str_arg.substr(0,13) != "--starting-T="&& str_arg.substr(0,8) != "--aaRSs=" && str_arg.substr(0,3) != "-N=" && str_arg.substr(0,10) != "--popsize=" && str_arg.substr(0,3) != "-k=" && str_arg.substr(0,3) != "-n=" && str_arg.substr(0,2) != "-1" && str_arg.substr(0,2) != "-0" && str_arg.substr(0,8) != "--trial=" && str_arg.substr(0,5) != "--bp=" && str_arg.substr(0,5) != "--Mu=" && str_arg.substr(0,13) != "--Site-Types=" && str_arg.substr(0,9) != "--fthalt="&& str_arg.substr(0,9) != "--fxhalt="&& str_arg.substr(0,3) != "-S="&&str_arg.substr(0,15) != "--codon-space-4"&& str_arg.substr(0,15) != "--codon-space-2" && str_arg.substr(0,8) != "--kappa=" && str_arg.substr(0,3) != "-i=" && str_arg.substr(0,8) != "--ifile="){
  ...
  if(bif){
    ...
    if(!bkmin)
      kdc = 220;
    if(kdnc <= kdc){
      cout<<"\n\nkdnc must be larger than kdc. Use -h or --help for more.\n\n";
      return 0;
    }
    if(k > n){
      cout<<"\nk must be at most n.\n\n";
  ...
  log_file<<"Genome-wide substitution rate: Mu = "<<Mu<<endl;
  cout<<"Mu (genome-wide substitution rate) is "<<Mu<<endl;
  ...
  //cout<<"t (time steps) is "<<nsteps<<endl;
  ...
  log_file<<"The non-cognate dissociation rate: kdnc = "<<kdnc<<endl;
  cout<<"kdnc is "<<kdnc<<endl;
  log_file<<"The cognate dissociation rate: kdc = "<<kdc<<endl;
  cout<<"kdc is "<<kdc<<endl;
  ...
  if(bseed){
    ...
    cout<<"PRNG seed: random device\n";
    log_file<<"PRNG seed: random device\n";
  }
  ...
  epsilon = (log(kdnc)-log(kdc))/float(k);
